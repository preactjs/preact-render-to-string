...
export default function renderToString(vnode, ctx) {
	...
	try {
		return _renderToString(vnode, context, parent);
	} finally {
...
function _renderToString(vnode, ctx, parent) {
	...
	let current = [[createStackItem(vnode, ctx, parent, false, undefined)], 0];
...
const TEXT_NODE_TYPE = 1 << 5;
const NULL_NODE_TYPE = 1 << 6;
